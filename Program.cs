...
using System.Linq;
using static GalaxyStat.CustomMethods;
                    ...
                    if (input.Contains("add"))
                    {
                        ...
                        else if (split[0] == "star")
                        {
                            ...
                            if (isFound)
                            {
                                ...
                                double lum = double.Parse(split[5]);
                                var star = new Star(starName, mass, size, temp, lum);
                                if (star.StarClass == 'X')
                                {
                                ...
                                foreach (var kvpStar in starDictionary)
                                {
                                    if (kvpStar.Key == name)
                                    {
                                        foreach (var star in kvpStar.Value)
                                        {
                                            ...
                                            Console.WriteLine($"      Class: {star.StarClass} ({star.Mass}, {star.Size}, {star.Temp}, {star.Lum})");
